...
export async function preDeployChecks(tronWeb: TronWeb, _allConfigs: L0Config[], _broadcastConfig: L0Config) {
    const endpointInstance = await tronWeb.contract(EndpointABI, tronWeb.address.fromHex(_broadcastConfig.endpoint));
    for (const config of _allConfigs) {
        ...
        // and aptos (30108)
        if (config.eid === 30370 || config.eid === 30376 || config.eid === 30375 || config.eid === 30367 || config.eid === 30325 || config.eid === 30108) continue
...
export async function deployContract(tronWeb: TronWeb, abi: any, bytecode: any, parameters: any[]): Promise<string> {
