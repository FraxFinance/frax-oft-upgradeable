...
// Set Linea peers to zero address
// forge script scripts/ops/fix/FixPeers/FixLineaSolanaPeers.s.sol --rpc-url https://rpc.linea.build
contract FixLineaSolanaPeers is DeployFraxOFTProtocol {
    ...
    function filename() public view override returns (string memory) {
        ...
        string memory name = string.concat(
            root,
            "/scripts/ops/fix/FixPeers/txs/FixLineaSolanaPeers.json"
        );
        return name;
    }
    ...
    function run() public override simulateAndWriteTxs(broadcastConfig) {
        for (uint256 i = 0; i < lineaProxyOfts.length; i++) {
            zeroAddressPeers.push(address(0));
        }

        setNonEvmPeers(lineaProxyOfts);
    }
