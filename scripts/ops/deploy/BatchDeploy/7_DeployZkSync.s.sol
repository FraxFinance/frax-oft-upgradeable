...
// forge script scripts/ops/deploy/BatchDeploy/7_DeployZkSync.s.sol --rpc-url https://mainnet.era.zksync.io --zksync
contract DeployZkSync is DeployFraxOFTProtocol {
            ...
            for (uint256 c=0; c<_configs.length; c++) {
                ...
                if (_configs[c].chainid == 252) {
                    ...
                    if (_connectedOfts[o] == frxUsdOft || _connectedOfts[o] == sfrxUsdOft) {
                        // 1. zksync => (ink, sonic, arbitrum, optimism, polygon, avalanche, bsc, polygon zkevm)
                        if (block.chainid == 324) { // TODO: DOUBLE CHECK THIS EVERY COPY PASTA
                            if (
                                ...
                                _configs[c].chainid != 56 && _configs[c].chainid != 1101
                            ) {
                                continue;
                            }
                        // 2. (ink, sonic, arbitrum, optimism, polygon, avalanche, bsc, polygon zkevm) => zksync
                        } else if (
                            block.chainid != 57073 && block.chainid != 146 &&
                            block.chainid != 42161 && block.chainid != 10 &&
                            block.chainid != 137 && block.chainid != 43114 &&
                            block.chainid != 56 && block.chainid != 1101
                        ) {
                            continue;
                        }
                    }
