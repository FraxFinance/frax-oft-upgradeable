...
// Deployment halted at set configs
// forge script scripts/ops/deploy/BatchDeploy/5a_SetupBSC.s.sol --rpc-url https://bsc-dataseed.bnbchain.org --broadcast
contract SetupBSC is DeployFraxOFTProtocol {
    ...
    // (s)frxUSD only to Fraxtal, Ink, Sonic, Arbitrum, Optimism, Polygon, Avalanche
            ...
            for (uint256 c=0; c<_configs.length; c++) {
                ...
                if (_configs[c].chainid == 252) {
                    ...
                    if (_connectedOfts[o] == frxUsdOft || _connectedOfts[o] == sfrxUsdOft) {
                        // 1. bsc => (ink, sonic, arbitrum, optimism, polygon, avalanche)
                        if (block.chainid == 56) { // TODO: DOUBLE CHECK THIS EVERY COPY PASTA
                            if (
                        ...
                        // 2. (ink, sonic, arbitrum, optimism, polygon, avalanche) => bsc
                        } else if (
